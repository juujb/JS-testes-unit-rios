...
const orderFromMenu = (request) => {
  if (restaurant.consumption.includes(request) === false) {
...
const foodPayment = () => {
  const food = restaurant.fetchMenu()[0];
  if (restaurant.fetchMenu().length === 0 || food.length === 0) {
    return undefined;
  }
  let totalPrice = 0;
  for (let index = 0; index < food.length; index += 1) {
    const foodPrice = food[index];
...
const drinksPayment = () => {
  const drinks = restaurant.fetchMenu()[1];
  if (restaurant.fetchMenu().length === 0 || drinks.length === 0) {
    return undefined;
  }
  let totalPrice = 0;
  for (let index = 0; index < drinks.length; index += 1) {
    const drinksPrice = drinks[index];
    totalPrice += drinksPrice;
  }
  return totalPrice;
};

const payment = () => (drinksPayment + foodPayment) * 1.1;
